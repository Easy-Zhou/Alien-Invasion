...
def update_screen(ai_settings, screen, stats, ship, alien, bullets, play_button):
    ...
    # 每次循环时都重绘屏幕
    screen.fill(ai_settings.bg_color)  # 绘制填充背景
    # 在飞船和外星人后面重新绘制所有子弹
    for bullet in bullets.sprites():
        bullet.draw_bullet()
    ship.blitme()  # 绘制飞船
    alien.draw(screen)  # 绘制外星人
...
def create_alien(ai_settings, screen, aliens, alien_number, row_number):
    ...
    # alien.y = alien.get_rect.height + 2 * alien.get_rect.height * row_number
